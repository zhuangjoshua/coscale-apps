...
/** The brand artwork the landing page uses. Kept here so the app header and the
 *  site cannot drift onto two different marks. */
export function brandLogoSrc(): string {
  const configured = String(
    (surfaceContext as Record<string, unknown>).brandLogoUrl || "",
  ).trim();
  return configured || "/brand-logo.png";
}
