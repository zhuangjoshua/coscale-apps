...
/**
 * Current Supabase access token, for calling the Pagewright document service.
 *
 * The document engine runs a headless browser, which an action-runtime backend cannot
 * host, so it lives as a separate service and authenticates with this bearer token.
 * Returns "" when auth is unconfigured — callers fall back to the service's dev mode.
 */
export async function getSupabaseAccessToken(): Promise<string> {
  try {
    const supabase = await getSupabaseBrowserClient(readSurfaceAuthConfig());
    if (!supabase) return "";
